...
墨流 vNext 现在可以完整完成：首次进入 → 选择场景 → 写下任务与回来第一步 → 手动/模拟等待 → 脑间歇 → 完成信号 → 一击接回 → 完成第一步 → 恢复质量复盘 → 本地历史。
...
- `app/flow/machine.mjs`：纯状态机，非法转换会明确抛错。
...
- BroadcastChannel 同步同源标签页；较旧 revision 不覆盖较新状态。
